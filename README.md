<!-- Renders on https://github.com/sansware -->

<div align="center">

# Jordan Gilbert

**Fractional CTO · Founder · EU-sovereign systems engineer**

I build EU-sovereign web, ticketing, and AI products, and I provide technical
leadership for startups scaling MVPs into production systems. Based in the United Kingdom.

[**Canonical author page →**](https://access.ticketwavehq.com/author/jordan-gilbert) · [**Everything I have written →**](https://access.ticketwavehq.com/author/jordan-gilbert/works) · [**RSS →**](https://access.ticketwavehq.com/author/jordan-gilbert/feed.xml)

[![Email](https://img.shields.io/badge/Email-jordan%40sansware.tech-22c55e?style=flat-square&labelColor=0b0620)](mailto:jordan@sansware.tech)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-eu--jordangilbert-0a66c2?style=flat-square&labelColor=0b0620)](https://www.linkedin.com/in/eu-jordangilbert/)
[![Instagram](https://img.shields.io/badge/Instagram-%40jordzaura-e4405f?style=flat-square&labelColor=0b0620)](https://www.instagram.com/jordzaura)
[![YouTube](https://img.shields.io/badge/YouTube-%40jordzaura-ff0000?style=flat-square&labelColor=0b0620)](https://www.youtube.com/@jordzaura)
[![Website](https://img.shields.io/badge/Site-sansware.tech-a78bfa?style=flat-square&labelColor=0b0620)](https://sansware.tech)
[![Companies House](https://img.shields.io/badge/Companies%20House-17143167-d6d3e0?style=flat-square&labelColor=0b0620)](https://find-and-update.company-information.service.gov.uk/company/17143167)

</div>

---

## 🏗 What I am building

| | Project | Role | Where |
|---|---|---|---|
| 🎫 | **[TicketWave HQ](https://github.com/TicketWaveHQ)** | Founder + Technical CTO | Access-decision infrastructure for ticketing, hospitality, and services. Stripe Connect, multi-vertical (events / food / bookings). → [ticketwavehq.com](https://ticketwavehq.com) |
| &nbsp; | &nbsp;&nbsp;↳ **Witness** | Sibling product | TicketWave Witness — physical security sensing plane that wires venues, doors, and crowd-flow signals into the same access-decision pipeline. |
| 🌍 | **[Citorah](https://citorah.com)** | Founder + Editor | Editorial city-guide network for Ibiza, Barcelona, Madrid, London, Lisbon, Greece, Dubai, and Cancun. Locals' concierge across cities, taught with love and sensitivity. → [citorah.com](https://citorah.com) |
| 🌐 | **[UK Web Marketing](https://github.com/Ukwebmarketing)** | Founder + Lead Engineer | Managed, EU-sovereign website infrastructure for UK businesses. Education-led, compliance-first, fully managed. → [ukwebmarketing.com](https://ukwebmarketing.com) |
| 🏥 | **[Orenva Health](https://github.com/Orenva-Health)** | Co-founder + CTO | Connective layer for healthcare, one health context across consultation, pharmacy, therapy, fitness, insurance, and wellness. EU-sovereign, India-first. Coming soon. → [orenvahealth.com](https://orenvahealth.com) |
| 📚 | **[Brother Sylvester](https://brothersylvester.com)** | Technical Maintainer | Afrocentric education for children and families. Academy LMS + CIO foundation, taught with love and sensitivity. → [brothersylvester.com](https://brothersylvester.com) |

---

## ✍️ Where I write

- **Research** (long-form, whitepapers): [access.ticketwavehq.com/research](https://access.ticketwavehq.com/research) — the flagship is [The Economics of White-Label Event Ticketing](https://access.ticketwavehq.com/research/whitelabel-economics)
- **Blog** (platform economics + engineering): [access.ticketwavehq.com/blog](https://access.ticketwavehq.com/blog)
- **Help centre** (op notes + product docs): [access.ticketwavehq.com/help](https://access.ticketwavehq.com/help)
- **City editorial** (Citorah): [citorah.com](https://citorah.com)

All of the above are cross-listed on [my author page](https://access.ticketwavehq.com/author/jordan-gilbert/works) and syndicated through a single [Atom feed](https://access.ticketwavehq.com/author/jordan-gilbert/feed.xml).

---

## 🛠 How I work

**Hands-on, single-engineer, AI-leveraged.** Every line of code I ship, or oversee, I can explain. I do not subcontract critical paths.

- **Stack default**: Next.js 16 + Astro 6 on Vercel · Neon Postgres · Stripe Connect · Resend · Cloudflare for DNS/edge · Sentry EU + PostHog EU
- **Frameworks I lean on**: TypeScript, Drizzle ORM (schema + migrations), Tailwind 4 (CSS-first), Turborepo
- **Compliance posture default**: EU-sovereign hosting, DKIM/SPF/DMARC alignment, DNSSEC, RFC 9116 security.txt
- **Engineering principle**: ship a thing that works end-to-end before adding the second feature

I am the engineer who answers the email. There is no team layer between us.

---

## 🎓 Background

- **BSc Architectural Engineering & Design Management** — Loughborough University
- **HarvardX GSD1x: The Architectural Imagination** — HarvardX ([verified certificate](https://courses.edx.org/certificates/614c26ddea004818a73d50ad8b264855)). An online course. Not a Harvard degree.
- **Twenty years building for the web** — since 2006

### Career history

Reverse chronological. Named-firm employment history, verifiable via [LinkedIn](https://www.linkedin.com/in/eu-jordangilbert/).

| When | Role | Where |
|---|---|---|
| Apr 2026 – present | **Founder + CTO** | [TicketWave HQ Ltd](https://ticketwavehq.com) — Leeds, UK, remote |
| Jan 2026 – present | **Founder + Editor** | [Citorah](https://citorah.com) |
| Apr – Aug 2023 | **Facilities Manager** | Castlepoint Markets Ltd |
| Jul – Nov 2022 | **Graduate Mechanical Engineer** | [Sweco](https://www.sweco.co.uk/) — London. HVAC + envelope optimisations delivering ~10% energy improvement, digital-twin prototype recognised by internal innovation award. |
| Sep 2019 – Mar 2020 | **Technical Design Manager** | [Higgins Partnerships](https://www.higginspartnerships.co.uk/) — Essex. £23M residential development delivered on time, 5% under budget. |
| Jul – Sep 2018 | **Technical Design Manager** | [Kier Group](https://www.kier.co.uk/) — Essex. Contributed to ~£60M pipeline of project wins, established design-QA process. |
| Jul – Aug 2017 | **Engineering Intern** | [Mott MacDonald](https://www.mottmac.com/) — Leeds |
| Aug 2016 | **Quality Assurance Intern** | [Alternative Software Ltd](https://www.alternativesoftware.com/) — Wakefield. Selenium-based QA testing, ~18% reduction in reported defects on release under test. |
| 2006 – present | **Founder** | [UK Web Marketing](https://ukwebmarketing.com) — personal project since primary school, incorporated in 2026 under TicketWave HQ Ltd |

An engineer by training who spent seven years across the UK's construction consultancies (Mott MacDonald, Kier, Higgins, Sweco) before consolidating twenty years of side-project web work into TicketWave HQ Ltd.

---

## 📬 Get in touch

| For | Where |
|---|---|
| **Fractional CTO retainer** | [ukwebmarketing.com/audit](https://ukwebmarketing.com/audit) (start with a free site audit) |
| **TicketWave partner application** | [ticketwavehq.com/partners/apply](https://ticketwavehq.com/partners/apply) |
| **Direct email** | [jordan@sansware.tech](mailto:jordan@sansware.tech) |
| **LinkedIn** | [linkedin.com/in/eu-jordangilbert](https://www.linkedin.com/in/eu-jordangilbert/) |
| **YouTube** | [@jordzaura](https://www.youtube.com/@jordzaura) |
| **Instagram** | [@jordzaura](https://www.instagram.com/jordzaura) |

---

<div align="center">

*Operating company: **TicketWave HQ Ltd** · Companies House [17143167](https://find-and-update.company-information.service.gov.uk/company/17143167) · England & Wales*

</div>

---

## 🔗 Find me elsewhere

[@TicketWaveHQ](https://github.com/TicketWaveHQ) · [@Ukwebmarketing](https://github.com/Ukwebmarketing) · [@Orenva-Health](https://github.com/Orenva-Health) · [LinkedIn](https://www.linkedin.com/in/eu-jordangilbert/) · [YouTube](https://www.youtube.com/@jordzaura) · [Instagram](https://www.instagram.com/jordzaura) · [sansware.tech](https://sansware.tech)

<!--
  Canonical Person JSON-LD. References the same @id emitted on
  access.ticketwavehq.com/about#jordan-gilbert so entity linkers
  consolidate GitHub authorship with the canonical Person entity.
  Cross-domain mirror lives on brothersylvester.com (maintainer),
  citorah.com (guides byline), and access.ticketwavehq.com (all
  editorial surfaces). See:
    - access.ticketwavehq.com/author/jordan-gilbert (ProfilePage)
    - access.ticketwavehq.com/author/jordan-gilbert/works (CollectionPage)
    - access.ticketwavehq.com/author/jordan-gilbert/feed.xml (Atom)
-->

```json
{
  "@context": "https://schema.org",
  "@type": "Person",
  "@id": "https://access.ticketwavehq.com/about#jordan-gilbert",
  "name": "Jordan Lewis Gilbert",
  "givenName": "Jordan",
  "familyName": "Gilbert",
  "additionalName": "Lewis",
  "url": "https://access.ticketwavehq.com/author/jordan-gilbert",
  "mainEntityOfPage": {
    "@type": "ProfilePage",
    "@id": "https://access.ticketwavehq.com/author/jordan-gilbert#profile"
  },
  "jobTitle": "Founder + CTO",
  "nationality": {"@type": "Country", "name": "United Kingdom"},
  "alumniOf": {"@type": "CollegeOrUniversity", "name": "Loughborough University", "url": "https://www.lboro.ac.uk/"},
  "sameAs": [
    "https://github.com/sansware",
    "https://www.linkedin.com/in/eu-jordangilbert/",
    "https://www.instagram.com/jordzaura",
    "https://www.youtube.com/@jordzaura",
    "https://sansware.tech"
  ],
  "worksFor": [
    {"@type": "Organization", "name": "TicketWave HQ Ltd", "url": "https://ticketwavehq.com"},
    {"@type": "Organization", "name": "Citorah", "url": "https://citorah.com"},
    {"@type": "Organization", "name": "UK Web Marketing", "url": "https://ukwebmarketing.com"},
    {"@type": "Organization", "name": "Orenva Health", "url": "https://orenvahealth.com"}
  ],
  "hasOccupation": [
    {"@type": "Role", "startDate": "2026-04", "roleName": "Founder + CTO", "affiliation": {"@type": "Organization", "name": "TicketWave HQ Ltd", "url": "https://ticketwavehq.com"}},
    {"@type": "Role", "startDate": "2026-01", "roleName": "Founder + Editor", "affiliation": {"@type": "Organization", "name": "Citorah", "url": "https://citorah.com"}},
    {"@type": "Role", "startDate": "2023-04", "endDate": "2023-08", "roleName": "Facilities Manager", "affiliation": {"@type": "Organization", "name": "Castlepoint Markets Ltd"}},
    {"@type": "Role", "startDate": "2022-07", "endDate": "2022-11", "roleName": "Graduate Mechanical Engineer", "affiliation": {"@type": "Organization", "name": "Sweco", "url": "https://www.sweco.co.uk/"}},
    {"@type": "Role", "startDate": "2019-09", "endDate": "2020-03", "roleName": "Technical Design Manager", "affiliation": {"@type": "Organization", "name": "Higgins Partnerships", "url": "https://www.higginspartnerships.co.uk/"}},
    {"@type": "Role", "startDate": "2018-07", "endDate": "2018-09", "roleName": "Technical Design Manager", "affiliation": {"@type": "Organization", "name": "Kier Group", "url": "https://www.kier.co.uk/"}},
    {"@type": "Role", "startDate": "2017-07", "endDate": "2017-08", "roleName": "Engineering Intern", "affiliation": {"@type": "Organization", "name": "Mott MacDonald", "url": "https://www.mottmac.com/"}},
    {"@type": "Role", "startDate": "2016-08", "endDate": "2016-09", "roleName": "Quality Assurance Intern", "affiliation": {"@type": "Organization", "name": "Alternative Software Ltd", "url": "https://www.alternativesoftware.com/"}},
    {"@type": "Role", "startDate": "2006", "roleName": "Founder", "affiliation": {"@type": "Organization", "name": "UK Web Marketing", "url": "https://ukwebmarketing.com"}}
  ]
}
```
